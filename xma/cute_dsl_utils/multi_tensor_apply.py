...
import torch
from cutlass import Int32, const_expr, range_constexpr
...
class MultiTensorApplyCUDAKernel:
    def __init__(self, BLOCK_SIZE: int, M: int, depth: int, num_tensors: int) -> MultiTensorApplyCUDAKernel:
    ...
    def kernel(
        ...
        tiled_copys: list[cute.TiledCopy],
        shape: cute.Shape,
                    ...
                    if local_tile < total_tiles_per_tensor:
                        ...
                        xs = [
                            _load(
                                ...
                                rC=rC,
                                thr_copy=thr_copy,
                                copy_atom=copy_atoms[d],
                                block_coord=block_coord,
                                is_within_boundary=is_within_boundary,
                            )
                            for d in range_constexpr(self.depth)
                        ]

                        ys = self.compute(xs)

                        for d in range_constexpr(self.depth):
                            if const_expr(ys[d] is not None):
                                _store(
                                    gY=gXss[d][t],
                                    y=ys[d],
                                    rC=rC,
    ...
    @cute.jit
    def __call__(self, mXss: list[list[cute.Tensor]], stream: cuda.CUstream) -> None:
        vector_size = min([128 // mXs[0].element_type.width for mXs in mXss])
        ...
        copy_atoms = [cute.make_copy_atom(cute.nvgpu.CopyUniversalOp(), mXs[0].element_type) for mXs in mXss]
        tiled_copys = [cute.make_tiled_copy_tv(copy_atom, thr_layout, val_layout) for copy_atom in copy_atoms]
...
def multi_tensor_apply(
    ...
    mXss = [[t.view(1, -1) for t in tensors] for tensors in tensor_lists]

    compiled_kernel = get_compiled_multi_tensor_apply_cuda_kernel(
        caller_op=caller_op,
        key=key,
        kernel_class=lambda: kernel_class(depth=depth, num_tensors=num_tensors),
        example_tensors_list=tuple(tuple(mXs) for mXs in mXss),
        divisibility_list_list=tuple(tuple(divisibility_list) for _ in range(depth)),
        stream=stream,
    )

    compiled_kernel(mXss, stream)
