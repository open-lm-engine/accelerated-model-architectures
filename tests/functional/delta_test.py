...
def prepare_data(
    ...
    v = torch.randn(B, T, HV, DV, generator=g, dtype=dtype, device=device)
    ...
    do = torch.randn(B, T, HO, DV, generator=g, dtype=dtype, device=device)
    return q, k, v, b, S, do
...
def run(HV: int, HO: int) -> None:
    ...
    v_maybe_broadcast = repeat(v, "b t h d -> b t (h g) d", g=H // HV).contiguous()
    do_maybe_broadcast = repeat(do, "b t h d -> b t (h g) d", g=H // HO).contiguous()
    o0, dq0, dk0, dv0, db0, dS0 = fwd_bwd(
        ...
        b=b,
        S=S,
        do=do_maybe_broadcast,
        scale=scale,
    )
    dv0 = reduce(dv0, "b t (h g) d -> b t h d", "sum", g=H // HV)
    ...
    assert o1.shape == (B, T, H, DV)
    assert dv1.shape == (B, T, HV, DV)
    torch.testing.assert_close(o1, o0)
    torch.testing.assert_close(dq1, dq0)
    torch.testing.assert_close(dk1, dk0)
    torch.testing.assert_close(dv1, dv0)
    torch.testing.assert_close(db1, db0)
    torch.testing.assert_close(dS1, dS0)


@pytest.mark.parametrize("HV", [1, 3, 21])
@pytest.mark.parametrize("HO", [1, 3, 21])
def test_delta_rule_broadcast(HV: int, HO: int) -> None:
    run(HV=HV, HO=HO)
...
def main() -> None:
    ...
    def run_cp(HV: int, HO: int) -> None:
        ...
        torch.testing.assert_close(dq1, flat(dq0)[:, t0:t1], rtol=1.3e-6, atol=1.5e-5)
        torch.testing.assert_close(dk1, flat(dk0)[:, t0:t1], rtol=1.3e-6, atol=2e-5)
        torch.testing.assert_close(dv1, flat(dv0)[:, t0:t1])
        torch.testing.assert_close(db1, flat(db0)[:, t0:t1], rtol=1.3e-6, atol=1.5e-5)
